...
/**
 * linear_skip - function searches for a value conatined in a skip list; assumes a
 ...
 * @list: param, pointer to the head of the skip list to traverse
 * @value: param, value to search
 * Return: pointer on the first node where value is located, else if list is NULL,
 * then NULL else value not found
 */
skiplist_t *linear_skip(skiplist_t *list, int value)
{
	...
	while (stop && stop->next != stop->express)
		stop = stop->next;
	/* search value is found between two express nodes */
	if (temp->express)
	{
		printf("Value checked at index [%lu] = [%i]\n",
		       temp->express->index, temp->express->n);
	...
	/* search value is greater than last express node, this is out of list */
	else
		printf("Value found between indexes [%lu] and [%lu]\n",
